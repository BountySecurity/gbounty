...
// Afero must implement the [scan.FileSystem] interface.
var _ scan.FileSystem = &Afero{}

// Afero is a [scan.FileSystem] implementation that uses the [afero.Fs] interface
...
//
// It also returns a function that can be used to close the iterator (see [scan.CloseFunc]).
// The channel is closed when the iterator is done (no more elements), when the [scan.CloseFunc]
...
func (a *Afero) LoadTasksSummaries(ctx context.Context) ([]scan.TaskSummary, error) {
	...
	var scanTasks []scan.TaskSummary
...
func (a *Afero) LoadTemplates(ctx context.Context) ([]scan.Template, error) {
	...
	var scanTemplates []scan.Template
	...
	for scanner.Scan() {
		var scanTemplate scan.Template
